...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (urlParams.get('login') === 'success') {
        showToast('Đăng nhập thành công!', 'success', 3000);
        window.history.replaceState({}, document.title, window.location.pathname);
    }

    // Ngăn thay đổi giá trị khi cuộn chuột trên các ô nhập số
...
function displayStudentProfile(student) {
    ...
    if (gpaEl) {
        gpaEl.textContent = student.gpa_accumulated !== undefined ? Number(student.gpa_accumulated).toFixed(2) : '0.00';
    }
    document.getElementById('profileAcademicClass').textContent = student.academic_class || 'Chưa xếp lớp';
...
function showToast(message, type = 'info', duration = 1500) {
    if (window.UIComponents && typeof window.UIComponents.showAlert === 'function') {
        ...
        window.UIComponents.showAlert(message, { type: normalizedType, duration });
        return;
    }
